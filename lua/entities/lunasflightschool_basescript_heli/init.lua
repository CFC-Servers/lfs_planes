...
-- TODO: Localize more frequently used functions

-- TODO: Use :IsValid where possible
...
-- TODO: Figure out how to reduce all of the creations of Vectors, Angles in this function
-- TODO: Localize a lot of the math and other frequently called functions
...
local rotorCollideSounds = {
	"ambient/materials/roust_crash1.wav",
...
function ENT:OnRotorCollide( Pos, Dir )
	...
	self:EmitSound( rotorCollideSounds[math.random( #rotorCollideSounds )] )
end
...
-- TODO: Simplify this process of avoiding stuff
local function avoidEnt( self, avoid, ent, myRadius, myDir )
	if not IsValid( ent ) then return end
	if not ent.LFS then return end

	local theirRadius = v:BoundingRadius() 
	local Sub = self:GetPos() - v:GetPos()
	local Dir = Sub:GetNormalized()
	local Dist = Sub:Length()

	if Dist >= ( theirRadius + myRadius + 200 ) then return end

	if math.deg( math.acos( math.Clamp( myDir:Dot( -Dir ) , -1, 1 ) ) ) >= 90 then return end

	avoid = avoid + Dir * ( theirRadius + myRadius + 500 )
end

local function avoidPlanes( self, avoid, foundPlanes )
	if not istable( foundPlanes ) then return end

	local myDir = self:GetForward()
	local myRadius = self:BoundingRadius() 

	for _, ent in ipairs( foundPlanes ) do
		if ent ~= self then
			avoidEnt( self, avoid, ent, myRadius, myDir )
		end
	end
end

-- TODO: Optimize this mess
-- At minimum pre-define the trace tables and update them as needed
function ENT:RunAI()
	...
	local FrontLeft = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[1] ):Forward() * RangerLength } )
	local FrontRight = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[2] ):Forward() * RangerLength } )
	
	local FrontLeft2 = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[3] ):Forward() * RangerLength } )
	local FrontRight2 = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[4] ):Forward() * RangerLength } )

	local FrontLeft3 = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[5] ):Forward() * RangerLength } )
	local FrontRight3 = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[6] ):Forward() * RangerLength } )

	local FrontUp = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[7] ):Forward() * RangerLength } )
	local FrontDown = util.TraceLine( { start = StartPos, filter = self, endpos = StartPos + self:LocalToWorldAngles( aiAngles[8] ):Forward() * RangerLength } )
	...
	local Down2 = util.TraceLine( { start = self:LocalToWorld( VEC_UP_100 ), filter = self, endpos = StartPos + Vector( 0, 0, -RangerLength ) } )

	local cAvoid = VEC_ZERO
	avoidPlanes( self, cAvoid, self.FoundPlanes )
