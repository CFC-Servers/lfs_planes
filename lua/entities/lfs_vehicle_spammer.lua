...
local AZURE = Color( 0, 127, 255, 255 )
...
function ENT:SetupDataTables()
	...
	for _, v in ipairs( AllSents ) do
		if v and istable( v.t ) and v.t.Spawnable and ( v.t.Base and string.StartWith( v.t.Base:lower(), "lunasflightschool_basescript" ) ) and ( v.t.Category and v.t.PrintName ) then
...
if SERVER then
	local spawnPosOffset = Vector( 0, 500, 150 ) 
	local spawnAngOffset = Angle( 0, -90, 0 )
	...
	function ENT:OnTakeDamage( dmginfo )
		...
		if NewHealth <= 0 and ( not self.MarkForRemove ) then
	...
	function ENT:Think()
		...
		if self.ShouldSpawn then
			if self.NextSpawn < CurTime() then
				...
				local pos = self:LocalToWorld( spawnPosOffset )
				local ang = self:LocalToWorldAngles( spawnAngOffset )
...
if CLIENT then
	...
	-- TODO: Reduce the amount of new Vectors created
	function ENT:Draw()
		self:DrawModel()
		...
		if not self:GetMasterSwitch() then
			...
			render.DrawSprite( self:LocalToWorld( Vector( 125, 500 - self.PX,10 ) ), 32, 32, WHITE )
			render.DrawSprite( self:LocalToWorld( Vector( -125, 500 - self.PX,10 ) ), 32, 32, WHITE )
			
			render.DrawSprite( self:LocalToWorld( Vector( 125, 500 - self.PX, 10 ) ), 130, 130, RED )
			render.DrawSprite( self:LocalToWorld( Vector( -125, 500 - self.PX, 10 ) ), 130, 130, RED )
		...
		render.DrawSprite( self:LocalToWorld( Vector( 125, 500 - self.PX, 10 ) ), 130, 130, AZURE )
		render.DrawSprite( self:LocalToWorld( Vector( -125, 500 - self.PX, 10 ) ), 130, 130, AZURE )
	end
	...
	hook.Add( "HUDPaint", "!!!!!!!11111lfsvehiclespammer_tutorial", function()
		...
		if Ent:GetClass() == "lfs_vehicle_spammer" then
			...
			if Ent:GetType() == "" then
				draw.SimpleText( "Hold C => Right Click on me => Edit Properties => Choose a Type", "LFS_FONT", scr.x, scr.y - 10, WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			else
				if not Ent:GetMasterSwitch() then
					...
					draw.SimpleText( "Now press [" .. Key .. "] to enable!", "LFS_FONT", scr.x, scr.y - 10, WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
					draw.SimpleText( "or hold [" .. Key .. "] to enable globally!", "LFS_FONT", scr.x, scr.y + 10, WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
				else
					TutorialDone = true
				end
			end
